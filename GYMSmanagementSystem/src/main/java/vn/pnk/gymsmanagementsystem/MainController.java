...
public class MainController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cus_table.setItems(customerListWrapper.getCustomerList());
    ...
    public void switchForm(ActionEvent event){
        if (event.getSource() == dashboard_btn){
            ...
            formMessage.setText("Search form");
        }
    }
    ...
    public void searchCustomerByName() {
        String customerName = search_field.getText().trim();
        ...
        try {
            ...
            if (!xmlFile.exists()) {
                showError("Customer data file (customers.xml) not found.");
                return;
            }
            ...
            XPath xPath = XPathFactory.newInstance().newXPath();
            String expression = "/customers/customer[contains(translate(name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), '" 
                + customerName.toLowerCase() + "')]";
            XPathExpression expr = xPath.compile(expression);

            NodeList customerNodes = (NodeList) expr.evaluate(doc, XPathConstants.NODESET);

            for (int i = 0; i < customerNodes.getLength(); i++) {
                Node customerNode = (Node) customerNodes.item(i);

                String customerID = getNodeValueByTagName(customerNode, "customerID");
                String name = getNodeValueByTagName(customerNode, "name");
                String gender = getNodeValueByTagName(customerNode, "gender");
                String phoneNumber = getNodeValueByTagName(customerNode, "phoneNumber");
                String address = getNodeValueByTagName(customerNode, "address");
                String status = getNodeValueByTagName(customerNode, "status");
                String schedule = getNodeValueByTagName(customerNode, "schedule");
                String startDateStr = getNodeValueByTagName(customerNode, "startDate");
                String endDateStr = getNodeValueByTagName(customerNode, "endDate");

                Customer newCustomer = new Customer(customerID, name, gender, phoneNumber, address, status, schedule, startDateStr, endDateStr);
                foundCustomers.add(newCustomer);
            }

            if (foundCustomers.isEmpty()) {
                showInfo("Result", "No customer found with the name: " + customerName);
            } else {
                Platform.runLater(() -> customerListWrapper.updateTable(foundCustomers));
            }

        } catch (Exception e) {
            showError("An error occurred while searching for customers.");
            e.printStackTrace();
        }
    }
